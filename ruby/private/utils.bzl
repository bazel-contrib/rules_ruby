...
set runfile_path=%~1
set abs_path=
for /F "tokens=1,* usebackq" %%i in (`%SYSTEMROOT%\system32\findstr.exe /l /c:"!runfile_path! " "%MF%"`) do (
  set abs_path=%%j
)
if "!abs_path!" equ "" (
  if exist "%~f0.runfiles\!runfile_path:/=\!" (
    set abs_path=%~f0.runfiles\!runfile_path:/=\!
  ) else if exist "%RUNFILES_DIR%\!runfile_path:/=\!" (
    set abs_path=%RUNFILES_DIR%\!runfile_path:/=\!
  )
)
if "!abs_path!" equ "" (
